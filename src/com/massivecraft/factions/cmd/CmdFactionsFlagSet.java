...
public class CmdFactionsFlagSet extends FactionsCommand
{
	...
	public void perform() throws MassiveException
	{
		...
		{
			msg("<b>Флаг <h>%s <b>не редактируется.", flag.getName());
			return;
		}
		...
		{
			msg("%s <i>уже имеет %s <i>установленные в %s<i>.", faction.describeTo(msender), flag.getStateDesc(value, false, true, true, false, true), flag.getStateDesc(value, true, true, false, false, false));
			return;
		}
		...
		if (msender.getFaction() != faction)
		{
			...
			msg("<h>%s <i>установить флаг для <h>%s<i>.", msender.describeTo(msender, true), faction.describeTo(msender, true));
			message(stateInfo);
		}
		faction.msg("<h>%s <i>установить флаг для <h>%s<i>.", msender.describeTo(faction, true), faction.describeTo(faction, true));
