...
public class CmdFactionsHome extends FactionsCommandHome
{
	...
	public void perform() throws MassiveException
	{
		...
		{
			msender.msg("<b>Извините, на этом сервере отключена возможность телепортироваться в дома фракций.");
			return;
		}
		...
		String homeDesc = "дом для " + faction.describeTo(msender, false);
		...
		if (home == null)
		{
			msender.msg("<b>%s <b>не имеет дома.", faction.describeTo(msender, true));
			
			if (MPerm.getPermSethome().has(msender, faction, false))
			{
				msender.msg("<i>Вам следует:");
		...
		{
			msender.msg("<b>Вы не можете телепортироваться на %s <b>дом, находясь на территории вражеской фракции.", homeDesc);
			return;
		}
		
		if ( ! MConf.get().homesTeleportAllowedFromDifferentWorld && !me.getWorld().getName().equalsIgnoreCase(home.getWorld()))
		{
			msender.msg("<b>Вы не можете телепортироваться на %s <b>дом, в то время как вы в другом мире.", homeDesc);
			return;
		}
		...
		)
		{
			...
			for (Player p : me.getServer().getOnlinePlayers())
			{
				...
				msender.msg("<b>Вы не можете телепортироваться на %s <b>дом, пока враг находится в пределах %f блоков от вас.", homeDesc, MConf.get().homesTeleportAllowedEnemyDistance);
				return;
			}
		}
