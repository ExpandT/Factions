	...
	public void perform() throws MassiveException
	{
		...
		{	
			...
			if ( ! isInvited)
			{
				...
				// Inform
				mplayer.msg("%s<i> пригласил вас в %s<i>.", msender.describeTo(mplayer, true), msenderFaction.describeTo(mplayer));
				msenderFaction.msg("%s<i> приглашенный %s<i> вашей фракции.", msender.describeTo(msenderFaction, true), mplayer.describeTo(msenderFaction));
			...
			else
			{
				...
				String tooltip = Txt.parse("<i>Нажмите для <c>%s<i>.", command);
				
				Mson remove = Mson.mson(
					mson("Вы могли бы хотеть удалить его. ").color(ChatColor.YELLOW),
					mson("Нажмите для " + command).color(ChatColor.RED).tooltip(tooltip).suggest(command)
				);
				
				// Inform
				msg("%s <i>уже приглашен %s<i>.", mplayer.getName(), msenderFaction.getName(msender));
				message(remove);
			}
		}
	}
