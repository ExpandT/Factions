...
public class CmdFactionsInviteList extends FactionsCommand
{
	...
	{		
		...
		final Pager<Entry<String, Invitation>> pager = new Pager<>(this, "Список приглашенных игроков", page, invitations, new Stringifier<Entry<String, Invitation>>()
		{
			public String toString(Entry<String, Invitation> entry, int index)
			{
				...
				String inviterDisplayName = inviterId != null ? MixinDisplayName.get().getDisplayName(inviterId, sender) : Txt.parse("<silver>неизвестный");
				...
				if (entry.getValue().getCreationMillis() != null)
				{
					...
					ageDesc = " " + ageDesc + Txt.parse(" тому назад");
				}

				return Txt.parse("%s<i> был приглашен %s<reset>%s<i>.", inviteeDisplayName, inviterDisplayName, ageDesc);
			}
		});
