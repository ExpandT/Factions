...
public class CmdFactionsList extends FactionsCommand
{
	...
	public void perform() throws MassiveException
	{
		...
		// Pager Create
		final Pager<Faction> pager = new Pager<>(this, "Список фракций", page, new Stringifier<Faction>() {
			@Override
			public String toString(Faction faction, int index)
			{
				if (faction.isNone())
				{
					return Txt.parse("<i>Factionless<i> %d онлайн", FactionColl.get().getNone().getMPlayersWhereOnlineTo(sender).size());
				}
				else
				{
					return Txt.parse("%s<i> %d/%d онлайн, %d/%d/%d",
